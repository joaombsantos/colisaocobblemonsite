...
class PaymentController {
    ...
    static async calculateCartTotal(nick) {
    ...
    static async createPixPayment(req, res) {
        ...
        try {
            ...
            const payment = new mercadopago_1.Payment(PaymentController.getMpClient());
    ...
    static async createCardPreference(req, res) {
        const { nick } = req.body;
        try {
            const total = await PaymentController.calculateCartTotal(nick);
            if (total <= 0)
                return res.status(400).json({ error: "Carrinho vazio ou inválido." });
            const preference = new mercadopago_1.Preference(PaymentController.getMpClient());
    ...
    // ==========================================
    static processingPayments = new Set();
    static async handleWebhook(req, res) {
        ...
        try {
            const paymentClient = new mercadopago_1.Payment(PaymentController.getMpClient());
            ...
            if (paymentInfo.status === "approved") {
                ...
                if (playerNick) {
                    ...
                    if (cart && cart.items.length > 0) {
                        ...
                        await EmailService_1.EmailService.sendPurchaseNotification(playerNick, total, cart.items);
                        console.log(`[PAGAMENTO APROVADO] Entregando itens para: ${playerNick}`);
                        await CommandService_1.CommandService.executeCartCommands(playerNick);
                    }
                    else {
